(() => {
  ...
  const css = `
    .comments-section{margin-top:30px;padding-top:22px;border-top:3px solid #0e7490}
    .comments-section h2{font-family:"Roboto Condensed",Arial,sans-serif;text-transform:uppercase;font-size:22px;margin:0 0 4px}
    ...
    .comment{background:#fff;border:1px solid #d8d8d8;padding:14px 16px;margin-bottom:12px;display:flex;gap:14px}
    ...
    .comment-form textarea,.comment-auth input{width:100%;background:#fff;border:1px solid #b9b9b9;color:#202020;padding:11px;font:inherit;font-size:16px}
    ...
    .comment-chars{color:#666;font-size:12px}.comment-submit{background:#22d3ee;color:#0b1524;border:2px solid #0b1524;padding:11px 20px;font-weight:900;text-transform:uppercase;cursor:pointer}
  ...
  async function mount(){
    ...
    root.innerHTML=`<h2>Comments (${comments.length})</h2><p class="comments-note">Comments are published straight away. Be civil, stick to the story, and remember real people live here.</p><div id="comment-compose"></div><div id="comment-list">${comments.map(commentMarkup).join('')||'<p class="comments-note">No comments yet. Be the first.</p>'}</div>`;
    renderCompose();
  }
  mount();
})();
