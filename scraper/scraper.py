...
def source_text_is_local(text: str, source_name: str='', source_url: str='', trusted_local: bool=False) -> bool:
    if source_is_denied(source_name, source_url):
        return False
    return bool(trusted_local or is_local(text, source_name, source_url) or rochdale_traffic_area(text))
...
def main() -> int:
    ...
    batches = {'rss_and_google_news': safe_collect('rss_and_google_news', collect_rss_candidates, collector_counts, collector_errors), 'website_discovery': safe_collect('website_discovery', collect_discovery_candidates, collector_counts, collector_errors), 'aggregator_discovery': safe_collect('aggregator_discovery', collect_aggregator_candidates, collector_counts, collector_errors), 'live_service_pages': safe_collect('live_service_pages', collect_live_page_candidates, collector_counts, collector_errors), 'facebook_events': safe_collect('facebook_events', collect_facebook_event_discovery_candidates, collector_counts, collector_errors), 'facebook_official': safe_collect('facebook_official', collect_facebook_candidates, collector_counts, collector_errors), 'x_official': safe_collect('x_official', collect_x_candidates, collector_counts, collector_errors), 'environment_agency': safe_collect('environment_agency', collect_environment_agency_flood_candidates, collector_counts, collector_errors)}
