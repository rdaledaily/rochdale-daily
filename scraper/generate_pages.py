...
# The CSS is embedded inline into every generated page (see SITE_CSS below)
# rather than linked as an external file. A missing/misplaced stylesheet on
# deploy would otherwise silently render every article page unstyled.
...
def source_is_denied(source_name: str, source_url: str) -> bool:
    ...
    domain = re.sub(r"^www\.", "", (re.findall(r"://([^/]+)", str(source_url or "")) or [""])[0]).lower()
...
def insert_incontent_ad(content_html: str) -> str:
    ...
    if len(parts) <= 1:
        return content_html
    ad_slot = (
        '<div class="ad-slot ad-slot-incontent" role="presentation" '
        'aria-hidden="true"></div>'
    )
    break_index = min(3, len(parts) - 1)
    parts.insert(break_index, ad_slot)
    # Restore the "</p>" that split() consumed, except for the trailing
    # empty fragment produced by the final split.
...
def related_stories_markup(article: dict[str, Any], all_articles: list[dict[str, Any]]) -> str:
    ...
    related = [
        ...
        if a.get("slug") != slug and str(a.get("category") or "news").lower() == category
    ]
    related = related[:3]
    if not related:
        return ""
    ...
    return (
        f'<div class="sidebar-box"><h3>More in {esc(category_label(category))}</h3>'
        + "".join(items)
        + "</div>"
    )
...
def json_ld(article: dict[str, Any], canonical_url: str, image_url: str) -> str:
    ...
    data = {
        ...
        "dateModified": article.get("scraped_at") or published,
        "author": {"@type": "Organization", "name": "Rochdale Daily"},
        "publisher": {
            "@type": "Organization",
            "name": "Rochdale Daily",
            "logo": {
...
def sources_markup(article: dict[str, Any]) -> str:
    ...
    names = article.get("source_names") or []
    if urls:
        items = "".join(
            f'<li><a href="{esc(url)}" target="_blank" rel="noopener noreferrer">'
            f'{esc((names[i] if i < len(names) else "") or f"Source {i + 1}")}</a></li>'
            for i, url in enumerate(urls)
        )
        return f'<div class="article-source"><strong>Sources used:</strong><ul>{items}</ul></div>'
    source_url = article.get("source_url")
    if source_url:
        source_name = esc(article.get("source_name") or "Original report")
        return (
            f'<p class="article-source"><strong>Source:</strong> '
            f'<a href="{esc(source_url)}" target="_blank" rel="noopener noreferrer">{source_name}</a></p>'
        )
    return ""
...
def write_sitemap(slugs_with_dates: list[tuple[str, str]]) -> None:
    ...
    entries = [
        f"  <url><loc>{esc(SITE_BASE_URL)}/</loc><lastmod>{now}</lastmod><changefreq>hourly</changefreq></url>"
    ]
    for slug, lastmod in slugs_with_dates:
        clean_lastmod = lastmod or now
        entries.append(
            f"  <url><loc>{esc(SITE_BASE_URL)}/articles/{esc(slug)}.html</loc>"
            f"<lastmod>{esc(clean_lastmod)}</lastmod><changefreq>daily</changefreq></url>"
        )
    xml = (
...
def main() -> None:
    ...
    print(
        f"Generated {written} new article page(s), "
        f"left {skipped_existing} existing page(s) untouched, "
