...
def main() -> None:
    ...
    if start < 0:
        raise SystemExit('old image-policy docstring start not found')
    end = text.find('"""', start + 3)
    if end < 0:
        raise SystemExit('old image-policy docstring end not found')
    end += 3
    ...
    if marker not in text:
        raise SystemExit('metadata helper insertion point not found')
    ...
    if old_policy not in text:
        raise SystemExit('old image report policy not found')
    ...
    if count != 1:
        raise SystemExit('non-cards regression test block not found')
    data, count = re.subn(
    ...
    if count != 1:
        raise SystemExit('full-run image policy test block not found')
